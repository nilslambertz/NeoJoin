 ...
 * @param dataTypes data types from the source models that are used in the target model (does not
 *     include Ecore data types)
...
public record AQR(
        ...
        AQRTargetClass root) {
    public List<AQRTargetClass> classesWithoutRoot() {
        return classes.stream().filter(clazz -> clazz != root).toList();
    }
}
